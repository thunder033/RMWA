...
angular.module('pulsar-warp', [])
    .service('WarpFieldCache', [function(){
        ...
        this.retrieve = (clip) => {
            return JSON.parse(localStorage.getItem(clip.name));
        }
    }])
    .service('Warp', function (MConcurrentOperation, AudioClipService, AudioData, AutoPlay, MediaStates, SampleCount, MScheduler, MEasel, AudioPlayerService, WaveformAnalyzer, MalletMath, WarpFieldCache) {
        var MM = MalletMath;
        ...
        function startLevel(clip, warpField){
         ...
         * @param clip
         */
        function generateWarpField(clip) {
            var warpField = {
                duration: 0,
                timeStep: NaN,
                gems: null
        ...
        this.init = () => {
            ...
            AudioClipService.getClipList() //wait for clips to load
                ...
                .then(function(){
                    var clip = AudioClipService.getAudioClip(AutoPlay);
                    if(clip.name == AutoPlay && clip.state == MediaStates.READY){
                        console.log('generating field');
                        //start the game with the default clip
                        return generateWarpField(clip).then(field => startLevel(clip, field));
                    }
            });
        };
